...
class ParkourAgent(OnboardAgent):
    # --- [修改点 1] 适配奥比中光 Gemini 2 的分辨率和帧率 ---
    # RealSense 原版: (480, 270) @ 60Hz
    # Orbbec Gemini 2: (640, 400) @ 30Hz
    rs_resolution = (640, 400)
    rs_frequency = 30
    ...
    def step(self):
        ...
        if self.debug_depth_publisher is not None:
            ...
            # --- [修改点 2] 修改调试可视化的 Frame ID，需与驱动层保持一致 ---
            depth_image_msg.header.frame_id = "orbbec_depth_link" 
            
            self.debug_depth_publisher.publish(depth_image_msg)
            
        if self.debug_pointcloud_publisher is not None:
            # 这里的 pointcloud_msg 内部使用驱动层 (ros_node) 的 frame_id，
            # 只要驱动层改了，这里自动生效
